...
void DriverOLEDPrintSmChar(unsigned char x, unsigned char y, unsigned char ch, uint8_t scr)
{
	...
	for (yi=y,yc=0;yi<y+8;yi++,yc++)
		for (xi=x,xc=0;xi<x+5;xi++,xc++)
			{
				if ((font_6x8[ch-32][xc]>>(7-yc)) & 0x01)
				DriverOLEDDrawPixel(xi,yi);
				else
				DriverOLEDClearPixel(xi,yi);
			}
...
void DriverOLEDInit(uint8_t Orientation)
{
	...
	while (1)
	{
		sprintf (sbuf,"CNT:%d",a);
		print_smtext(3,sbuf,0);
		update();
		a++;
	}
	...
	drawPixel(LCD_Y_RES-1,0);
	drawPixel(LCD_Y_RES-1,LCD_X_RES-1);
	drawPixel(0,LCD_X_RES-1);
	
	//print_text(0,"ABCDE");
	print_smtext(3,"abcdefg",0);
	//print_smchar(0,0,'a',0);
